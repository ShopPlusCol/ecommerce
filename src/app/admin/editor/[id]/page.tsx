...
import { AddBlockForm, BlockEditor, PageSettingsForm, PublishForm, RestoreVersionForm } from "../editor-forms";
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <main className="grid gap-6">
          ...
          <section><div className="mb-3"><h2 className="text-lg font-semibold">Bloques del borrador</h2><p className="text-sm text-text-muted">Reordena, duplica y edita. Publicar es una acción separada.</p></div>{sections.length ? <div className="grid gap-4">{sections.map((section, index) => <BlockEditor key={section.id} pageId={id} block={section} position={index} total={sections.length} media={media.map((item) => ({ url: item.url, label: item.altText || item.url }))} />)}</div> : <div className="rounded-xl border border-dashed border-border-strong p-10 text-center"><h3 className="font-semibold">El borrador está vacío</h3><p className="mt-1 text-sm text-text-muted">Agrega una portada, colección o sección de contenido para comenzar.</p></div>}</section>
        </main>
