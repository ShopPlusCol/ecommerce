...
export default async function Page() {
  ...
  return (
    <>
      ...
      <AdminEntitySection entity="shippingZone" title="Zonas geográficas" description="País, departamento o ciudad. La regla más específica siempre gana." fields={zoneFields} records={geoZones.map((row) => ({ ...row }))} recordTitle={(record) => `${record.name} · ${adminStatusLabel(String(record.level))} · ${adminStatusLabel(String(record.status))}`} canDuplicate={false} />
      <p className="mb-3 max-w-3xl rounded-lg border border-info/30 bg-info-soft p-4 text-sm text-info">
        Los barrios que crees aquí, con Nivel “Barrio” y su Ciudad, aparecen automáticamente como opción en el
        checkout de esa ciudad (por ejemplo Medellín) y pueden tener su propia regla de envío más abajo.
      </p>
      <AdminEntitySection entity="shippingZone" title="Barrios con tarifa propia" description="Barrios o sectores específicos dentro de una ciudad." fields={zoneFields} records={neighborhoodZones.map((row) => ({ ...row }))} recordTitle={(record) => `${record.name} · ${record.city ?? "sin ciudad"} · ${adminStatusLabel(String(record.status))}`} canDuplicate={false} />
